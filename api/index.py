"""
Backend FastAPI - Sistema de Homologação (Vercel Serverless)
API REST para integração com frontend React
Autor: Kauan Kelvin
Versão: 3.0.0 - Vercel Edition
"""
...
# Adicionar diretório atual ao path (para importar core/)
...
async def root():
    ...
    return {
        "status": "online",
        "message": "Sistema de Homologação API v3.0 - Vercel Edition",
        "author": "Kauan Kelvin",
        "docs": "/docs"
    }
...
async def generate_document_endpoint(data: DocumentoRequest):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Erro ao salvar no banco: {str(e)}")
...
async def generate_pdf_endpoint(data: DocumentoRequest):
    """Gera documento de atestado médico em PDF"""
    try:
        logger.info("Recebendo requisição para gerar PDF")
        
        # Salvar no banco (mesmo código do endpoint anterior)
        try:
            from sqlalchemy import text
