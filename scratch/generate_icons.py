...
# SVG do Estetoscópio em tom Cinza Slate Metallic (#cbd5e1 / #94a3b8 / #64748b)
...
# SVG do Logo Completo NOVA Medicina em tom Cinza Slate Metallic
LOGO_SVG_RAW = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="200" height="200">
  ...
  <g filter="url(#strongGlow)">
    <rect x="90" y="60" width="20" height="80" rx="6" fill="url(#slateGrad)"/>
    <rect x="70" y="90" width="60" height="20" rx="6" fill="url(#slateGrad)"/>
  </g>
  
  <!-- Certificate Document -->
  <g filter="url(#glow)">
    <rect x="125" y="50" width="45" height="60" rx="4" fill="none" stroke="url(#slateGrad)" stroke-width="3"/>
    <line x1="132" y1="65" x2="163" y2="65" stroke="url(#slateGrad)" stroke-width="2" stroke-linecap="round"/>
    <line x1="132" y1="75" x2="163" y2="75" stroke="url(#slateGrad)" stroke-width="2" stroke-linecap="round"/>
    <line x1="132" y1="85" x2="155" y2="85" stroke="url(#slateGrad)" stroke-width="2" stroke-linecap="round"/>
  </g>
  
  <!-- Stethoscope (Elegant) -->
  <g filter="url(#glow)" stroke="url(#slateGrad)" stroke-width="4" fill="none" stroke-linecap="round">
    <path d="M 35 75 Q 25 60, 40 48"/>
    <circle cx="40" cy="42" r="6" fill="url(#slateGrad)"/>
    <path d="M 40 48 L 40 90 Q 40 100, 50 100 L 70 100"/>
  </g>
  
  <!-- Health Monitor Pulse -->
  <g filter="url(#glow)" stroke="url(#slateGrad)" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
    <path d="M 30 150 L 50 150 L 58 135 L 66 165 L 74 150 L 170 150"/>
  </g>
  
  <!-- Accent Elements -->
  <circle cx="100" cy="100" r="92" stroke="url(#slateGrad)" stroke-width="2" fill="none" opacity="0.3"/>
  <circle cx="100" cy="100" r="85" stroke="url(#slateGrad)" stroke-width="1" fill="none" opacity="0.2"/>
  
  <!-- Glow Dots -->
  <circle cx="145" cy="115" r="3" fill="url(#slateGrad)" opacity="0.8" filter="url(#glow)"/>
  <circle cx="55" cy="115" r="3" fill="url(#slateGrad)" opacity="0.8" filter="url(#glow)"/>
</svg>"""
...
def render_stethoscope_icon(size, stroke_color="#94a3b8", stroke_width=2.2, fill_color="#cbd5e1", bg_style="dark", padding_ratio=0.18):
    """
    Renders a stethoscope icon in Slate/Metallic Grey:
    Dark BG: #1e293b -> #0f172a with #475569 border
    """
    ...
    if bg_style == "dark":
        # Fundo escuro grafite/cinza (#1e293b para #0f172a) com borda em #475569
        rect = QRectF(0, 0, size, size)
        grad = QLinearGradient(0, 0, size, size)
        grad.setColorAt(0.0, QColor(30, 41, 59))     # #1e293b
        grad.setColorAt(1.0, QColor(15, 23, 42))     # #0f172a
        
        painter.setBrush(QBrush(grad))
        pen = QPen(QColor(71, 85, 105, 200))        # Borda em #475569
        ...
        rect = QRectF(0, 0, size, size)
        grad = QLinearGradient(0, 0, size, size)
        grad.setColorAt(0.0, QColor(30, 41, 59))
        grad.setColorAt(1.0, QColor(15, 23, 42))
        painter.setBrush(QBrush(grad))
...
def generate_all_icons():
    print("[+] Atualizando TODOS os ícones, marcas d'água e logos na paleta CINZA (Slate Metallic)...")
    ...
    print("  - logo_light.png e logo_dark.png atualizados com paleta Cinza Slate Metallic")
    ...
    print("  - Ícones PWA (192, 512, maskable, apple-touch) gerados com sucesso")
    ...
    if os.path.exists("assets/app_icon(1).ico"):
        ico_frames[0].save(
            "assets/app_icon(1).ico",
            format="ICO",
            append_images=ico_frames[1:],
            sizes=[(s, s) for s in sizes]
        )
    ...
    print("[+] Sucesso! Todos os ícones, favicons e logos PWA/Desktop foram atualizados para a cor CINZA!")
